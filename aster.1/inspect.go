...
func (f *File) collectCommonTypes() {
	f.collectTypeSpecs(func(node *ast.TypeSpec, doc *ast.CommentGroup) {
		name := node.Name.Name
		t, ok := f.newCommonTypes(name, node.Type, doc, node.Assign != token.NoPos)
		if ok {
			f.Types[name] = t
		}
	})
...
func (f *File) newCommonTypes(name string, node ast.Expr, doc *ast.CommentGroup, isAssign bool) (t Type, ok bool) {
	var pkgName = f.PkgName
	switch x := node.(type) {
	case *ast.SelectorExpr:
		// alias type
		return newAliasType(x, name, pkgName, doc, newInvalidCommonType(f.tryFormat(x)), isAssign), true
	...
	case *ast.Ident:
		...
		if ok {
			...
			// alias type
			return newAliasType(x, name, pkgName, doc, t, isAssign), true
		}
		return newAliasType(x, name, pkgName, doc, newInvalidCommonType(x.Name), isAssign), true
	...
	case *ast.StarExpr:
		return f.newCommonTypes(name, x.X, doc, isAssign)
	default:
	}
	return
}
