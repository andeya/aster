...
type Config struct {
	...
	//
	// It must be safe to call concurrently from multiple goroutines.
	AfterTypeCheck func(info *PackageInfo, files []*File)
}
...
type PkgSpec struct {
	...
	Files     []*File  // ASTs of already-parsed files
...
type PackageInfo struct {
	...
	Files                 []*File // syntax trees for the package's files
...
//
func (conf *Config) CreateFromFiles(path string, files ...*File) {
...
func (conf *Config) Load() (*Program, error) {
	...
	createPkg := func(path, dir string, files []*File, errs []error) {
...
//
func (conf *Config) parsePackageFiles(bp *build.Package, which rune) ([]*File, []error) {
...
//
func (imp *importer) addFiles(info *PackageInfo, files []*File, cycleCheck bool) {
